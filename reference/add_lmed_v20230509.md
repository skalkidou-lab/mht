...
2023 MHT study. \`add_lmed_v20230509()\` categorises dispensed product
names into MHT groups. It corrects the dispensed durations of products
whose recorded defined daily doses are unreliable. It then derives the
approach-based treatment variables.
...
- lmed:
  ...
  - \`p1163_lopnr_personnr\` - person identifier, matched against
    \`skeleton\$id\`
...
reads the LMED identifier from \`p1163_lopnr_personnr\`. It has no
\`verbose\` argument: it always reports progress with \`message()\`.
