...
#
# THE ANCHOR DATE IS LEAD, NOT PACKAGING. 2016-01-03 is a SUNDAY, and every
# duration this file pins is a multiple of 7 (1680, 1008, and n * 28 from the
# minimum-dose loop). The dispensing interval therefore ENDS on a Sunday, on
# the last day of an ISO week, so adding ONE day moves the endpoint into the
# next week and the pin sees it.
#
# Verified by mutation: with this anchor, 1680 -> 1681 and 1008 -> 1009 both
# turn the relevant tests red. With a Monday anchor they do not. Weekly
# granularity is therefore NOT an inherent limit of the exported surface, and
...
test_that("the frozen tables cover every rung and every codebook product", {
  # COVERAGE BOOKKEEPING. `ladder_rungs()` is used here and nowhere else, and
  # it supplies names only, never a category.
  #
  # This block IS deliberately contractual, and it is the one place in the file
  # that fails on an addition rather than on a behaviour change. That is the
  # intent: a new rung or a new codebook product that nobody pinned is exactly
  # the gap a pinning suite exists to report. The repair is one line in the
  # frozen table, generated by running the classifier on the new name.
  for (nm in names(ladders)) {
    expect_setequal(
      unique(ladder_rungs(ladders[[nm]])$pattern),
      names(FROZEN_PATTERN_CATEGORY)
    )
  }
...
test_that("the IUD rule overrides the dispensed fddd", {
  ...
  # one day longer lands in the next ISO week, and the pin sees it
